...
class PrSt(object):
    ...
    def load_prs(self):
        return self.load_all("%s/repos/%s/pulls?token=%s" % (base, self.repo_name, config.token))

    def load_comments(self, pr_number):
        return self.load_all("%s/repos/%s/issues/%d/comments?token=%s" %
                             (base, self.repo_name, pr_number, config.token))
        ...
        for pr in prs:
            ...
            if self.in_sprint(pr["created_at"]) or self.in_sprint(pr["merged_at"]):
                res = self.gh.get("%s/repos/%s/pulls/%d?token=%s" %
                                  (base, self.repo_name, pr["number"], config.token))
                pr = res.json()
            ...
            diff_url = pr["diff_url"] + "token=" + config.token
